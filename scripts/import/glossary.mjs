...
const T = [
  ...
  { id: "zugfestigkeit", cat: "mechanik", unit: "MPa",
    term: t("Zugfestigkeit", "Tensile strength"), aliases: ["Reißfestigkeit", "Tensile"],
    short: t("Die höchste Spannung, die ein Prüfstab aushält, bevor er nachgibt oder reißt.",
    ...
    pitfall: t("Höhere Zugfestigkeit heißt nicht robusteres Bauteil. PLA hat mehr Zugfestigkeit als ABS und zerspringt trotzdem beim ersten Sturz — dafür zählt die Schlagzähigkeit.",
  ...
  { id: "bruchdehnung", cat: "mechanik", unit: "%",
    term: t("Bruchdehnung", "Elongation at break"), aliases: ["Reißdehnung", "Dehnung"],
  ...
  { id: "schlagzaehigkeit", cat: "mechanik", unit: "kJ/m²",
    ...
    detail: t("Gemessen mit einem Pendelhammer, entweder nach Charpy (liegend) oder Izod (eingespannt). GEKERBT heißt: Der Prüfstab hat eine eingefräste Kerbe — das simuliert eine scharfe Innenecke im Bauteil und liefert regelmässig nur einen Bruchteil des ungekerbten Wertes.",
    ...
    pitfall: t("Gekerbte und ungekerbte Werte stehen oft in derselben Tabelle und unterscheiden sich um den Faktor zehn. Wer sie verwechselt, verschätzt sich um eine Größenordnung. Für Bauteile mit Innenecken zählt der gekerbte Wert.",
  ...
  { id: "spannungsriss", cat: "mechanik",
    ...
    detail: t("Der heimtückischste Ausfallmechanismus: Weder die Last allein noch das Medium allein richten Schaden an. Polycarbonat reißt nach Wochen an einer Flächendesinfektion, die es beim Wischtest unbeschadet übersteht. Gedruckte Teile sind besonders anfällig, weil die Abkühlung Eigenspannungen einbaut.",
  ...
  { id: "hdt", cat: "thermik", unit: "°C",
    ...
    pitfall: t("HDT-B ist die geschmeichelte Zahl, und genau die steht im Marketing. Ein Datenblatt, das HDT-A nennt, macht es sich schwerer und ist deshalb aussagekräftiger. Beides ist außerdem KEINE Dauergebrauchstemperatur — die liegt deutlich darunter.",
  ...
  { id: "dauergebrauchstemperatur", cat: "thermik", unit: "°C",
    ...
    detail: t("Die einzige Größe, die für die Auslegung wirklich taugt — und die fast kein Filamenthersteller angibt. Sie liegt deutlich unter HDT, weil hier nicht nur Erweichung zählt, sondern auch die langsame thermische Alterung.",
  ...
  { id: "tempern", cat: "verarbeitung",
    ...
    pitfall: t("Ohne Umluftofen sind diese Werte schlicht nicht erreichbar. Deshalb fragt der Assistent danach — und blendet Werkstoffe aus, deren Zahlen ohne Ofen eine Illusion wären. Tempern kostet außerdem Massgenauigkeit: Das Bauteil schrumpft.",
  ...
  { id: "warping", cat: "verarbeitung",
    ...
    detail: t("Der begrenzende Faktor bei großen Bauteilen — nicht die Festigkeit. Je größer die Grundfläche und je höher die Schwindung, desto stärker. Bei XXL-Teilen entscheidet die Verzugsneigung darüber, was in einem Stück überhaupt fertigbar ist.",
  ...
  { id: "konfidenz", cat: "methodik",
    ...
    detail: t("Vier Stufen: mehrfach belegt (Herstellermessung an gedruckten Prüfkörpern), einfach belegt (Herstellerangabe ohne angegebenen Prüfkörper), schwach belegt (belegt, aber mit dokumentiertem Vorbehalt) und geschätzt (fachlich abgeleitet, ohne Primärquelle). Geschätzte Werte sind in der Oberfläche sichtbar markiert.",
              "Four levels: substantiated by several sources (manufacturer measurement on printed specimens), substantiated by one source (manufacturer figure without a stated specimen), weakly substantiated (sourced but with a documented caveat) and estimated (professionally derived, no primary source). Estimated values are visibly marked in the interface."),
    pitfall: t("Rund zwei Drittel der Aussagen sind gekennzeichnete Schätzungen. Das klingt nach wenig, ist aber ehrlicher als eine Lücke, die im Filter wie eine Freigabe aussieht. Ein als belegt geführter Wert ohne echte Quelle wird von der automatischen Prüfung zurückgewiesen.",
               "About two thirds of statements are flagged estimates. That sounds like little, but it is more honest than a gap that looks like a clearance in the filter. A value recorded as substantiated without a real source is rejected by the automated check."),
